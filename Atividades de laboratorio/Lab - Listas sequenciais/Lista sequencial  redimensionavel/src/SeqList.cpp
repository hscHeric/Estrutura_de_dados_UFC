...
Se n for maior que a capacidade atual do vetor, a fun¸c˜ao faz com que a lista
realoque seu armazenamento aumentando sua capacidade.
Em todos os outros casos, a chamada da fun¸c˜ao n˜ao causa uma realoca¸c˜ao e a
capacidade do vetor n˜ao ´e afetada.*/
void SeqList::reserve(int n){
    if(n > this->capacity_vec){
        int *new_vec = new int[n];
        for(int i = 0; i < this->size_vec; i++){
...
void SeqList::push_back(int elemento){
    if(this->size_vec == this->capacity_vec){
        this->reserve(this->capacity_vec * 2);
    }
    this->vec[this->size_vec] = elemento;
    this->size_vec++;
}
//Insere um elemento na posição i da Lista, somente se 0 <= i <= size()
void SeqList::insert(int elemento, int i)
{
    if(i < 0 || i > this->size_vec){
        throw std::out_of_range("Indice invalido");
    }
    if(this->size_vec == this->capacity_vec){
        this->reserve(this->capacity_vec * 2);
    }
    for(int j = this->size_vec; j > i; j--){
        this->vec[j] = this->vec[j - 1];
    }
    this->vec[i] = elemento;
    this->size_vec++;
